...
export function AreaView(props: Props): JSX.Element {
    ...
    const factor = createMemo((): number => 5 / viewerContext.viewer.scale);
    ...
    const transform = createMemo((): string => {
        ...
        const scale = viewerContext.viewer.scale;
    ...
    const polygon = createMemo((): JSX.Element => {
        ...
        const fill = viewerContext.viewer.mode == VIEWER_MODE.DEVELOPMENT
            ? "#0003"
            : "#0000";
    ...
    const helpers = createMemo((): JSX.Element[] => {
        if (viewerContext.viewer.mode == VIEWER_MODE.DEVELOPMENT) {
