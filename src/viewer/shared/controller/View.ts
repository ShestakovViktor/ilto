import type {Mat3} from "@src/shared/math";
import {type Scene, Tween, type Frame} from "@src/viewer/shared/controller";
...
export class View {
	...
	constrain(frame: Frame, scene: Scene): void {
		const deltaX = frame.w - scene.width * this.sTween.value;
		const minX = Math.min(deltaX, 0);
		const maxX = Math.max(deltaX, 0);
		this.applyBounce(this.xTween, minX, maxX);

		const deltaY = frame.h - scene.height * this.sTween.value;
