import en from "./string/en.json";
import * as styles from "./FigureBrowser.module.scss";

import {JSX} from "solid-js";
import i18next from "i18next";
import {AssetBrowser} from "@src/editor/widget";
import {Modal, Dialog} from "@src/core/widget";
import {AssetKind} from "@src/core/enum";
import {FigureForm} from "@src/editor/widget";
import {useEditorContext} from "@src/editor/context";

i18next.addResourceBundle("en", "prop", {"PropSelect": en}, true, true);

type Props = {
    selected?: number[];
    onSelect?: (ids: number[]) => void;
};

export function FigureBrowser(props: Props): JSX.Element {
    const {storage} = useEditorContext();

    const FigureFormDialog = new Modal();
    FigureFormDialog.render(
        <Dialog
            classList={{Dialog: styles.FigureFormDialog}}
            onClose={() => FigureFormDialog.hide()}
        >
            <FigureForm onSubmit={() => FigureFormDialog.hide()}/>
        </Dialog>
    );

    return (
        <AssetBrowser
            type={AssetKind.Graphics}
            selected={props.selected}
            onCreate={() => {
                FigureFormDialog.show();
            }}
            onDelete={(ids) => {
                ids.forEach((id) => {
                    storage.data.asset.delete(id);
                });
            }}
            onSelect={props.onSelect}
        />
    );
}